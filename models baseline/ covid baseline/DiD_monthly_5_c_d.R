...
es_list <- list()
cat("\n--- STARTING DYNAMIC RUN (COVID 2020) ---\n")
...
for (y in OUTCOMES[OUTCOMES %in% names(dat)]) {
  ...
  if (!is.null(ret)) {
    res <- ret$res
    # add GOF diagnostics to the per-outcome result
    res[, r2 := ret$r2]
    res[, ar2 := ret$ar2]
    res[, bp_p := ret$bp_p]
    es_list[[y]] <- res
...
try({
  for (y in unique(es_dynamic$outcome)){
    ...
    # Convert label to numeric only for pure numeric labels (avoid coercion warnings)

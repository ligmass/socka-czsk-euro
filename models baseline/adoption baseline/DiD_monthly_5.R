...
run_one_series <- function(dat, y, break_date){
  ...
  ## Omit NAs from outcome and trend only (no controls)
...
# ---------- write ----------
fwrite(es_main, file.path(OUT_DIR_RAW, "es_robust.csv"))
export_academic_table(es_main, file.path(OUT_DIR_FINAL, "es_robust_academic.csv"))
cat("\nDone. Outputs in ", OUT_DIR_RAW, ":\n- es_robust.csv (binned ES with HAC SE, GOF and trend; no controls)\n", sep = "")
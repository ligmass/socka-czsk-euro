...
## MODIFIED: The "fake" placebo date ##
TREAT_DATE <- as.IDate("2008-01-01")   # PLACEBO (FAKE) DATE
...
run_one_series <- function(dat, y, break_date){
  ...
  if (VERBOSE) {
    lbl <- sprintf("Outcome: %s (diff = CZ − SK) [Adoption Placebo (2008) %s]", y, as.character(break_date))
...
cat("\n--- STARTING DEBUG RUN (PLACEBO TEST 2008) ---\n")
...
fwrite(es_main, file.path(OUT_DIR_RAW, "es_placebo_2008_placebo.csv"))
export_academic_table(es_main, file.path(OUT_DIR_FINAL, "es_placebo_2008_placebo_academic.csv"))
cat("\nDone. Outputs in ", OUT_DIR_RAW, ":\n- es_placebo_2008_placebo.csv (Adoption Placebo (2008) coefficients with GOF columns)\n", sep = "")
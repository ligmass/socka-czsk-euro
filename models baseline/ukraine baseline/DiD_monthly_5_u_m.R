...
## ----------------------------------------
## MODIFIED FUNCTION: MECHANISM ANALYSIS WITH CONTROLS
## ----------------------------------------
run_one_series <- function(dat, y, break_date){
  ...
  if (VERBOSE) {
    lbl <- sprintf("Outcome: %s (diff = CZ − SK) [UKRAINE WITH POLICY AND FX %s]", y, as.character(break_date))
...
cat("\n--- STARTING DEBUG RUN (UKRAINE WITH POLICY AND FX TEST) ---\n")
...
fwrite(es_main, file.path(OUT_DIR_RAW, "es_ukraine_mechanism.csv"))
export_academic_table(es_main, file.path(OUT_DIR_FINAL, "es_ukraine_mechanism_academic.csv"))
cat("\nDone. Outputs in ", OUT_DIR_RAW, ":\n- es_ukraine_mechanism.csv (mechanism analysis with controls; coefficients with GOF columns)\n", sep = "")
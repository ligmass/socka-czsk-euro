...
# PLACEBO DATE: One year before the war
TREAT_DATE <- as.IDate("2021-02-01")
...
print_block <- function(lbl, dt, L, Tn, gof_stats = NULL, pre_W = NULL, post_W = NULL){
  ...
  if (!is.null(gof_stats)) cat(sprintf("R-squared: %.4f, Adj. R-squared: %.4f\n", gof_stats$r2, gof_stats$ar2))
  print(dt)
  if (!is.null(pre_W)) cat(sprintf("\n-- Joint pretrend --\nK=%d, chi2=%.4f, p=%g\n", pre_W$K, pre_W$stat, pre_W$p))
  if (!is.null(post_W)) cat(sprintf("\n-- Joint post period --\nK=%d, chi2=%.4f, p=%g\n", post_W$K, post_W$stat, post_W$p))
}
...
run_one_series <- function(dat, y, break_date){
  ...
  s <- summary(fit)
  gof <- data.table(outcome = y, n = nobs(fit), r2 = s$r.squared, ar2 = s$adj.r.squared)
  ...
  lbl <- sprintf("Outcome: %s (diff = CZ − SK) [Ukraine Placebo (2021) %s]", y, as.character(break_date))
...
es_list <- list()
gof_list <- list()
cat("\n--- STARTING UKRAINE PLACEBO TEST (2021) ---\n")
...
fwrite(es_main, file.path(OUT_DIR_RAW, "es_ukraine_placebo_2021_placebo.csv"))
export_academic_table(es_main, file.path(OUT_DIR_FINAL, "es_ukraine_placebo_2021_placebo_academic.csv"))
cat("\nDone. Outputs in", OUT_DIR_RAW, ":\n- es_ukraine_placebo_2021_placebo.csv (Ukraine Placebo (2021) coefficients with GOF columns)\n")
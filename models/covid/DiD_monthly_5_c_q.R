...
OUT_DIR   <- "tables5_covid_quad"; dir.create(OUT_DIR, showWarnings = FALSE, recursive = TRUE)

OUTCOMES  <- c("hicp_yoy","unemp_rate","hicp","imports_world_meur","exports_world_meur","log_imp","log_exp")

## MODIFIED: COVID-19 Date ##
...
## ----------------------------------------
## MODIFIED FUNCTION: Quadratic Trend + Policy Control
## ----------------------------------------
run_one_series <- function(dat, y, break_date){
  ...
  # Standard Controls (policy_diff + fx_vol)
  ...
  if (VERBOSE) {
    lbl <- sprintf("Outcome: %s (diff = CZ − SK) [COVID QUAD %s]", y, as.character(break_date))
...
cat("\n--- STARTING DEBUG RUN (COVID QUADRATIC TREND TEST) ---\n")
...
fwrite(es_main, file.path(OUT_DIR, "es_covid_quad.csv"))
fwrite(gof_main, file.path(OUT_DIR, "es_covid_quad_gof.csv"))

...
OUT_DIR   <- "tables5_ukraine_quad"; dir.create(OUT_DIR, showWarnings = FALSE, recursive = TRUE)
...
run_one_series <- function(dat, y, break_date){
  ...
  cz_controls <- dat[country == "CZ", .(date, repo, fx_vol)]; sk_controls <- dat[country == "SK", .(date, mro)]
  d0 <- merge(d0, cz_controls, by = "date", all.x = TRUE); d0 <- merge(d0, sk_controls, by = "date", all.x = TRUE); d0[, policy_diff := repo - mro]
  ...
  keep_cols <- c("diff", "bin", "trend", "policy_diff", "fx_vol"); d0 <- d0[!is.na(bin)]; d0 <- na.omit(d0, cols = keep_cols)
...
for (y in OUTCOMES[OUTCOMES %in% names(dat)]) { res <- run_one_series(dat, y, TREAT_DATE); if (!is.null(res) && nrow(res$coefs) > 0) es_list[[y]] <- res$coefs }
es_main <- rbindlist(es_list); fwrite(es_main, file.path(OUT_DIR, "es_ukraine_quad.csv")); cat("\nDone.\n")